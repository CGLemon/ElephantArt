...
UCTNode *UCTNode::uct_select_child(const Types::Color color,
    ...
    auto fp_factor                  = is_root ? parameters()->forced_policy_factor : 0.f;
    ...
    for (const auto &child : m_children) {
        ...
        const float fp = std::max(std::sqrt(fp_factor * psa * (float)parentvisits) - (denom-1.f), 0.f);
        const float puct = cpuct * psa * (numerator / denom);
        const float value = q_value + puct + fp;
